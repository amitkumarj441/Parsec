...
object ParsecProtocol {
  ...
  /**
    * Parsec Protocol lies here!
    *
    * To ensure that the <b>order</b> of Signed Invoices is guaranteed
    * No cluster of distributed systems can guarantee that (due to varying network latencies etc)
    *
    * So within the Parsec network - need to preserve a chain of HashPointers that provide ordering guarantees
    */
